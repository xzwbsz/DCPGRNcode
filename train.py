...
if os.path.exists('data/feature_2160nodes.npy') and os.path.exists('data/time_2160nodes.npy'):
    ...
    timestamp = np.load('data/time_2160nodes.npy')
else:
    ...
    for fdir in sorted(glob.glob(dataset_fp)):
        ...
        if month >= start_month and month <= end_month:
            ...
            for f in sorted(glob.glob(fdir)):
                print(f)
                dataset_pre = Dataset_pre(station_node,station_num,f)
...
in_dim = train_data.feature.shape[-1] + train_data.t2m.shape[-1] # 3
    ...
    if True:
        ...
        for epoch in range(config['idgl']['max_epochs']):
            if config['idgl']['graph_model'] == 'ls-gnn':
                model = ModelHandler(config, train_loader, val_loader, test_loader, adj0, dist)
