...
# 真实节点文件位置
# site_fp = os.path.join(proj_dir, 'data/2160nodes.CSV')
site_fp = os.path.join(proj_dir, 'data/142nodes.CSV')
# site_fp = os.path.join(proj_dir, 'data/178nodes.CSV')

class Graph():
    def __init__(self):
        self.dist_thres = 3 # 300km 3
    ...
    def _gen_edges(self):
        ...
        self.dist = distance.cdist(coords,coords,'euclidean') # 得到各点间的欧式距离 shape=(node_num，node_num)
        ...
        # 稠密矩阵转稀疏矩阵
        edge_index,dist = dense_to_sparse(torch.tensor(dist)) # edge_index [起始节点,到达节点]  dist 节点间的欧式距离
