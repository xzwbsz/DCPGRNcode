...
class IDGL(nn.Module):
    ...
    def __init__(self, config, adj0=None, gso=None, lonlat=None):
        ...
        if self.graph_module == 'ls-gnn':
            ...
            self.encoder = CLCRNModel(loc_info=self.loc_info, sparse_idx=self.sparse_idx, geodesic=self.geodesic, angle_ratio=self.angle_ratio, config=config)
        else:
        ...
        if self.graph_learn:
            ...
            self.graph_learner = graph_learn_fun(feature_num, config['idgl']['graph_learn_hidden_size'], # input_size=3 | hidden_size = 70
    ...
    """
        来自 module_handler.py 调用
            ...
            输出 A(t) 和 A_hat(t)
    """
    def learn_graph(self, graph_learner, gl_feature , graph_skip_conn=None, graph_include_self=False, init_adj=None, dist=None,anchor_features=None, ): # node_features 原始特征X或者Z(t-1) | init_adj A(0)初始邻接矩阵
        # 是否进行图学习
        if self.graph_learn:
            # 是否采用anchor
            if self.scalable_run:
                ...
                raw_adj = graph_learner(gl_feature,dist=dist) # A(t) = GL(Z(t-1))

                # assert raw_adj.min().item() >= 0 # 确定权重矩阵中没有负数项
                adj = raw_adj / torch.clamp(torch.sum(raw_adj, dim=-1, keepdim=True), min=VERY_SMALL_NUMBER) # f(A(t)) = f(A_ij) = A_ij/Σ_j(A_ij) 行归一化 (2708,2708)

                if graph_skip_conn in (0, None):
                    if graph_include_self:
                        adj = adj + to_cuda(torch.eye(adj.size(0)), self.device)
                else:
                    adj = graph_skip_conn * to_cuda(init_adj,self.device) + (1 - graph_skip_conn) * adj # λA(0) + (1-λ)f(A(t))
